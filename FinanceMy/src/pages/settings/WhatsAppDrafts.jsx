...
export default function WhatsAppDrafts({ user }) {
  ...
  return <section className="wa-drafts">
    <div className="wa-drafts-heading"><div><h3>Draf dari WhatsApp</h3><p>Periksa nominal, akun, dan budget sebelum transaksi memengaruhi saldo.</p></div><button type="button" className="secondary-btn" onClick={async () => { setLoading(true); try { setDrafts((await getWhatsAppDrafts()).drafts); setError('') } catch (reason) { setError(reason.message) } finally { setLoading(false) } }}><RotateCw size={14}/>Muat ulang</button></div>

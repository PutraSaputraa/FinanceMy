...
export default function BudgetsPage() {
  ...
  const daily = dailyBudgetSummary(budgets, month)
  ...
  return <>
    <PageTitle eyebrow="RENCANA BULANAN" title="Budget" subtitle="Tetapkan batas pengeluaran per kategori untuk bulan ini." action={<div className="page-actions budget-actions"><span className="budget-period"><CalendarDays/>{month.label}</span><button className="primary-btn" onClick={() => setOpen(true)}><Plus/>Buat budget</button></div>}/>
    <section className="budget-explainer card" aria-label="Cara kerja budget"><strong>Cara kerja budget</strong><p>Pilih kategori dan batas bulanan. Pengeluaran pada kategori itu otomatis mengurangi sisa budget. Budget hanya aktif selama {month.label}; bulan depan buat budget baru sesuai kebutuhanmu.</p></section>
    ...
    {budgets.length > 0 && <section className="adaptive-banner"><div className="adaptive-icon"><Sparkles/></div><div><span>PANDUAN HARIAN</span><h2>Hari ini sekitar {formatCurrency(daily.availableToday)}</h2><p>Gabungan panduan harian dari budget aktifmu. Pengeluaran hari ini hanya menghitung kategori yang memiliki budget.</p></div><div><small>Pengeluaran hari ini</small><strong>{formatCurrency(todayExpense)}</strong><span>Sisa panduan hari ini {formatCurrency(daily.availableToday - todayExpense)}</span></div></section>}
    {budgets.length ? <div className="budget-page-grid">{budgets.map((budget) => <BudgetCard key={budget.id} budget={budget} onDelete={(item) => { setDeleting(item); setDeleteError('') }}/>)}</div> : <div className="card empty-state"><Sparkles/><h3>Belum ada budget bulan ini</h3><p>Buat budget untuk mulai memantau pengeluaran per kategori.</p></div>}
    <Modal open={open} onClose={() => setOpen(false)} title="Buat budget bulan ini" description="Pilih kategori, nominal, dan cara menghitung panduan harian."><BudgetForm onDone={() => setOpen(false)}/></Modal>

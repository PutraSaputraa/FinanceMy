...
export default function TransactionForm({ onDone }) {
  ...
  const { register, handleSubmit, watch, formState: { errors, isSubmitting } } = useForm({ defaultValues: { date: '2026-08-04', time: '12:00', needType: 'kebutuhan', account: accounts[0]?.name } })
  ...
  return <form className="finance-form" onSubmit={handleSubmit(submit)}>
    ...
    <div className="form-grid">
      <label>{type==='income'?'Akun tujuan':'Akun pembayaran'}<select {...register('account',{required:true})}>{accounts.map((account)=><option key={account.id}>{account.name}</option>)}</select></label>
      {type === 'transfer' && <label>Akun tujuan<select {...register('destinationAccount',{validate:(value)=>value!==source||'Akun tujuan harus berbeda.'})}><option value="">Pilih akun</option>{accounts.map((account)=><option key={account.id}>{account.name}</option>)}</select>{errors.destinationAccount&&<small className="field-error">{errors.destinationAccount.message}</small>}</label>}
